...
/** @param {NS} ns */
async function shorthandIfTest(ns) {
  let scriptTitle = "Shorthand If Test";
  let logger = new F42Logger(ns, false, true, false, scriptTitle, true);
  let scriptDescription = "";
  let scriptFlags = [];
  let feedback = logger.initFeedback(scriptTitle, scriptDescription, scriptFlags);
  if (!feedback) {
...
function testNums(ns) {
  const rnd = Math.random();
  let rndThou = Math.floor(rnd * 1000);
  let rndRemainder = rndThou % 2;
...
/** @param {NS} ns */
async function targetServerTest(ns) {
  let logger = new F42Logger(ns, true, false, true, "HMV3", false);
  let scriptTitle = "HackManager V3 TEst";
  let scriptDescription = "";
  let scriptFlags = [];
  let feedback = logger.initFeedback(scriptTitle, scriptDescription, scriptFlags);
  if (!feedback) {
...
/** @param {NS} ns */
function thrallInfectorTest(ns) {
  ns.tprintf("FILES: >>>>>>>>>>>>>>");
