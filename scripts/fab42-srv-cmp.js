...
export async function main(ns) {
  ...
  let addAsTarget = ns.args[1] || false;
  ...
  let serverHackLev = ns.getServerRequiredHackingLevel(opVars.targetSeverHostname);
  let serverHackPorts = ns.getServerNumPortsRequired(opVars.targetSeverHostname);
  ...
  if (hasRoot) {
    ...
    if (addAsTarget) {
      // add as target
      F42HackManager.addTargetServer(ns, opVars.targetSeverHostname);
  ...
  let resultStr = "";
  let canHack = (serverHackLev <= userHackLev && serverHackPorts <= userMaxHackPorts);
